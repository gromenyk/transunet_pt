...
# Inicializa wandb (si este script se ejecuta separado del entrenamiento)
wandb.init(project="deeppatella", entity="deeppatella", name="centroids_visualization")
...
for npz_file in os.listdir(npz_folder):
    ...
    # Imagen con centroides sobre la predicción
    fig, ax = plt.subplots()
    ...
    wandb.log({
        "Original Image with Centroids": wandb.Image(output_path, caption=npz_file),
        "Prediction Heatmap with Centroids": wandb.Image(output_path_predicted, caption=npz_file)
    })

    print(f'Process finished: {output_path}')

wandb.finish()  # Cierra la sesión de wandb
